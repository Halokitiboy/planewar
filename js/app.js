	...
	function Bg(config){
		/*
		    定义构造函数的属性
			* 低耦合 - 构造函数的属性与数据初始化没有必然联系
			   * 如果将构造函数的属性值，直接定义数据初始化内容
			      * 一旦修改数据初始化的相关内容，构造函数必须一并修改（高耦合）
			   * 利用了函数的形参特点
			      * 构造函数的属性值与数据初始化并没有必然联系
		  */
	...
	function Bullet(config){
		...
		// 绘制方法
		this.paint = function(cxt){
			cxt.drawImage(this.imgs,this.x-10,this.y);
			cxt.drawImage(this.imgs,this.x+10,this.y);
		}
		// 移动方法
		this.step = function(){
	...
	setInterval(function(){
		...
		switch (state){
			case STARTTING://欢迎阶段
				// 绘制游戏LOGO
				context.drawImage(logoImg,40,0);
				break;
			case START://过渡阶段
